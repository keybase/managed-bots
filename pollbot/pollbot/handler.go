...
func (h *Handler) generateVoteLink(convID chat1.APIConvID, msgID chat1.MessageID, choice int) string {
	vote := NewVote(convID, msgID, choice)
	link := h.httpPrefix + "/pollbot/vote?=" + url.QueryEscape(vote.Encode())
	return strings.ReplaceAll(link, "%", "%%")
}

func (h *Handler) generateAnonymousPoll(convID chat1.APIConvID, msgID chat1.MessageID, prompt string,
...
func (h *Handler) generatePoll(convID chat1.APIConvID, msgID chat1.MessageID, prompt string,
	options []string) error {
...
func (h *Handler) handlePoll(cmd string, convID chat1.APIConvID, msgID chat1.MessageID) error {
